...
if __name__ == "__main__":
    with gr.Blocks(title="全流程处理", css="./css/gradio.css") as app:
        ...
        model_dir_dropdown.change(
            fn=update_model_folders, inputs=[], outputs=[all_3_text, model_dir_dropdown]
        )
        infer_path_dropdown.change(
            fn=update_g_files, inputs=[], outputs=[all_4_text, infer_path_dropdown]
        )
        config_path_dropdown.change(
            fn=update_c_files, inputs=[], outputs=[all_4_text, config_path_dropdown]
        )
        slice_btn.click(
            slice_audio,
            inputs=[
