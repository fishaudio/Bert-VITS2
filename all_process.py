...
def voice_detection_with_pydub(
    ...
    for i, segment in enumerate(segments):
        ...
        if (
            segment_duration >= min_wav_len * 1000
            and segment_duration <= max_wav_len * 1000
        ):
            output_filename = os.path.join(
                target_path_dir,
                os.path.basename(denoised_audio_path)
                .replace(".WAV", ".wav")
                .replace(".wav", f"_seg_{i}.wav"),
            )
...
def denoise_audio(
    ...
    denoise_audio_dir = os.path.join(
        "./denoised_audio", os.path.basename(target_path_dir)
    )
    ...
    for file in filelist:
        ...
        separated_path = (
            f"./separated/{character_folder_name}/htdemucs/{folder_name}/vocals.wav"
        )
        ...
        if os.path.exists(separated_path):
            ...
            if bool_detach:
                if os.name == "nt":
                    python_path = os.path.abspath("./env/python.exe")
                else:
                    python_path = "python"
                demucs_command = (
                    python_path
            ...
            if not os.path.exists(denoise_audio_path):
                wav, sr = torchaudio.load(
                    separated_path if bool_detach else raw_audio_path,
                    frame_offset=0,
                    num_frames=-1,
                    normalize=True,
                    channels_first=True,
    ...
    for file in filelist:
        if file.lower().endswith(".wav"):
            ...
            voice_detection_with_pydub(
                os.path.join(denoise_audio_dir, file),
                target_path_dir,
                min_wav_len=min_wav_len,
            ...
            if fname.lower().endswith(".wav"):
                with sf.SoundFile(os.path.join(target_path, fname)) as sound:
                    if len(sound) / sound.samplerate > max_wav_len:
                        ...
                        wav_file_to_remove = fname.replace(".WAV", ".wav").replace(
                            ".wav", ".lab"
                        )
                        if os.path.exists(
                            ...
                            result_str += (
                                f"[Too long] Deleted {fname} "
                                f"because it's longer than {max_wav_len} seconds.\n"
                            )
            elif fname.lower().endswith(".lab"):
                with open(os.path.join(target_path, fname), "r", encoding="utf-8") as f:
                    content = f.read()
...
def process_video_files(file, target_path, lang):
    ...
    video_dir = os.path.join(
        os.path.dirname(file_name), "video_dir", os.path.basename(target_path)
    )
    os.makedirs(video_dir, exist_ok=True)
    os.path.join(video_dir, os.path.basename(file_name))
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        executor.map(
            clip_file, videos, [video_dir] * len(videos), [target_path] * len(videos)
        )
    print("解析视频文件完成")
    return update_status.update_raw_folders()
    ...
    with gr.Blocks(title="全流程处理", css="./css/gradio.css") as app:
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        audio_folder = gr.inputs.File(
                            label="上传单个音频压缩包(.zip)", type="file", file_count="single"
                        )

                    with gr.Column():
                        video_folder = gr.inputs.File(
                            label="上传单个视频压缩包(.zip)", type="file", file_count="single"
                        )
                with gr.Row():
                ...
                with gr.Row():
                    ...
                    with gr.Column(min_width=100):
                        audio_submit_btn = gr.Button(
                            value="1.1 解析音频压缩包", variant="primary"
                        )
                    with gr.Column(min_width=100):
                        video_submit_btn = gr.Button(
                            value="1.2 解析视频压缩包", variant="primary"
                        )
                ...
                with gr.Row():
                    slider_trans_workers = gr.Slider(
                        minimum=1, maximum=12, value=1, step=1, label="转写进程数"
                    )
                ...
                with gr.Row():
                    ...
                    with gr.Column(min_width=150):
                        textbox_transcript_num = gr.Textbox(
                            label="对应转写文本数/音频数",
                ...
                with gr.Row():
                    transcript_btn = gr.Button(
                        value="1.4 提取总转写文本到filelists下", variant="primary"
                    )
                with gr.Row():
                    with gr.Column():
                    ...
                    with gr.Column():
                        textbox_output_text = gr.Textbox(
                            label="1.4 输出信息", placeholder="", lines=3
                        )
            with gr.Column():
                image_1 = gr.Image(
                    value=os.path.abspath("./img/神里绫华.png"),
                    show_label=False,
        ...
        with gr.Row():
            ...
            with gr.Column():
                gr.HTML(
                    '<h2><a href="https://space.bilibili.com/47278440">访问我的bilibili主页</a></h2>'
                )
            with gr.Column():
                pass
            ...
            with gr.Column():
                with gr.Row():
                    resample_btn = gr.Button(
                        value="2.1 音频重采样到44100hz(音频从raw转到dataset)", variant="primary"
                    )
                with gr.Row():
                    preprocess_text_btn = gr.Button(
                        value="2.2 生成训练集和验证集标注文本", variant="primary"
                    )
                with gr.Row():
                    slider_bert_gen = gr.Slider(
                        value=2, step=1, label="bert处理线程数", minimum=1, maximum=12
                    )
                with gr.Row():
                    bert_gen_btn = gr.Button(
                        value="2.3 生成bert文件(语调、停顿)", variant="primary"
                    )
                with gr.Row():
                    all_2_text = gr.Textbox(label="2 输出信息", placeholder="", lines=3)
            with gr.Column():
                image_2 = gr.Image(
                    value=os.path.abspath("./img/宵宫.png"),
                    show_label=False,
                    show_download_button=False,
                )
        ...
        with gr.Row():
            ...
            with gr.Column():
                gr.HTML(
                    '<h2><a href="https://github.com/AnyaCoder">访问我的github主页(求晶哥放过)</a></h2>'
                )
            with gr.Column():
                pass
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        model_dir_dropdown = gr.Dropdown(
                            label="选择模型文件夹名称(即底模所在的位置，没有则会在./logs下创建文件夹)",
                            value="mix",
                            allow_custom_value=True,
                            interactive=True,
                        )
                with gr.Row():
                    slider_batch_size = gr.Slider(
                        minimum=1, maximum=40, value=4, step=1, label="batch_size 批处理大小"
                    )
                ...
                with gr.Row():
                    stop_train_btn = gr.Button(
                        value="终止训练（已弃用，请手动关闭窗口）", variant="secondary"
                    )
            ...
            with gr.Column():
                image_3 = gr.Image(
                    value=os.path.abspath("./img/纳西妲.png"),
                    show_label=False,
                    show_download_button=False,
                )
        # ----------------------------------------------------------------------
        ...
        with gr.Row():
            ...
            with gr.Column():
                gr.HTML(
                    '<h2><a href="https://www.codewithgpu.com/u/spicysama">访问我的autodl主页</a></h2>'
                )
            with gr.Column():
                pass
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        infer_path_dropdown = gr.Dropdown(
                            label="选择模型G_xxxx.pth(请放在logs的某个文件夹下)", interactive=True
                        )
                    with gr.Column():
                        config_path_dropdown = gr.Dropdown(
                            label="选择配置文件config.json(请放在logs的某个文件夹下)", interactive=True
                        )
                with gr.Row():
                    infer_btn = gr.Button(value="4.1 点击开始推理", variant="primary")
                with gr.Row():
                    stop_infer_btn = gr.Button(
                        value="终止推理（已弃用，请手动关闭窗口）", variant="secondary"
                    )
                with gr.Row():
                    all_4_text = gr.Textbox(label="4 输出信息", placeholder="")
            with gr.Column():
                image_4 = gr.Image(
                    value=os.path.abspath("./img/yuyu.png"),
                    show_label=False,
                    show_download_button=False,
                )
        # ----------------------------------------------------------------------
        model_dir_dropdown.change(
            fn=update_status.update_model_folders,
            inputs=[],
            outputs=[all_3_text, model_dir_dropdown],
        )
        infer_path_dropdown.change(
            fn=update_status.update_g_files,
            inputs=[],
            outputs=[all_4_text, infer_path_dropdown],
        )
        config_path_dropdown.change(
            fn=update_status.update_c_files,
            inputs=[],
            outputs=[all_4_text, config_path_dropdown],
        )
        tar_path_dropdown.change(
            fn=update_status.update_raw_folders,
            inputs=[],
        ...
        transcript_btn.click(fn_transcript, inputs=[], outputs=[textbox_output_text])
        preprocess_text_btn.click(
            extern_subprocess.do_preprocess_text, inputs=[], outputs=[all_2_text]
        )
        resample_btn.click(
            extern_subprocess.do_resample, inputs=[], outputs=[all_2_text]
        )
        bert_gen_btn.click(
        ...
        stop_train_btn.click(
            extern_subprocess.terminate_training, inputs=[], outputs=[all_3_text]
        )
        train_btn.click(
        ...
        stop_infer_btn.click(
            extern_subprocess.terminate_webui, inputs=[], outputs=[all_4_text]
        )
        infer_btn.click(
