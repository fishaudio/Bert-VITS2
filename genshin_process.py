                    ...
                    if fname.endswith('.wav'):
                        with sf.SoundFile(os.path.join(target_path, fname)) as sound:
                            if len(sound) / sound.samplerate > 10:
                                ...
                                if os.path.exists(os.path.join(target_path, wav_file_to_remove)):
                                    rm_files.append(os.path.join(target_path, wav_file_to_remove))
    ...
    with gr.Blocks(title="处理音频文件/压缩包") as app:
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    textbox_tar_path = gr.Textbox(label="提取目的路径（可修改,精确到角色文件夹名)",
                                                  placeholder="输入目的文件夹路径",
                                                  value=args.target_path, interactive=True)
                with gr.Row():
                    with gr.Column():
                ...
                with gr.Row():
                    textbox_transcript = gr.Textbox(label="可以提取的角色音频/状态",
                                                    value=update_transcript_status(textbox_tar_path.value))
                    dropdown_lang = gr.Dropdown(label="选择语言", choices=list(lang_dict.keys()), value="ZH(中文)")
                with gr.Row():
        ...
        transcript_btn.click(
            fn_transcript,
            inputs=[textbox_tar_path],
            outputs=[textbox_transcript]
        )
